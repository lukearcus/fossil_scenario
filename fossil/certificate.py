...
class Direct_control_barr(Certificate):
    """
    ...
    def compute_state_loss(self, V_U, V_I):
        ...
        unsafe_loss = -V_U #minus since V_I<0
        N_data = len(unsafe_loss)
        acc = (sum(unsafe_loss < 0))/(N_data)
        loss = relu(unsafe_loss+margin).mean()
    ...
    def learn(
        ...
        states_only = torch.cat([samples[idot1:i1], samples[i1+idot2:i1+i2], samples[i1+i2+idot3:i1+i2+i3]])
        times = times[XD]
        ...
        for t in range(learn_loops):
            if state_sol:
                ...
                # Coordinate descent: optimize each control dim in turn (see
                # Direct_control inner loop for full comment).
        ...
        # Final re-evaluation uses the coordinate-descent grid-min V-next.
    ...
    def get_violations(self, nets, S, state_data):
        ...
        req_diff = -(nets[0](state_data["unsafe"]).min()-nets[0](state_data["init"]).max())/self.T
...
class Direct_control_RWA(Certificate):
    """
    Certificies stability for CT and DT models
    bool LLO: last layer of ones in network
    XD: Symbolic formula of domain
    ...
    def compute_state_loss(self, V_D, V_G, V_I, V_U, beta):
        ...
        unsafe_loss = -V_U
        acc = (acc*N_data+ sum(unsafe_loss < 0))/(2*N_data)
        loss = loss + relu(unsafe_loss+margin).mean()
    ...
    def compute_loss(
        ...
        loss = V_next-V+req_diff_2
        ind_losses_2 = torch.reshape(loss[stacked_inds], (len(indices["lie"]),-1))
        
        losses_2 = torch.max(ind_losses_2, axis=1)[0].squeeze()
        if ind_losses_2.shape[0] > 1:
            acc = sum(losses <= 0)/len(losses)
        else:
            acc = (losses <= 0)
        losses = torch.max(losses,losses_2)
        return losses, {"traj_acc":acc.item()*100}
    ...
    def learn(
        ...
        samples = torch.cat([S[XD], S[XI], S[XG_BORDER],  S[XG], S[XU]])
        f_samples = f_samples[XD]
        g_samples = g_samples[XD]
        samples_dot = Sdot[XD]
        ...
        for t in range(learn_loops):
            if state_sol:
                ...
                loss, state_acc = self.compute_state_loss(V_D, V_G, V_I, V_U, beta)
                acc = learn_accuracy|state_acc
                losses = relu(losses) + loss
                #if loss > 0:
                #    losses = relu(losses) + loss
    ...
    def get_violations(self, nets, S, state_data):
        ...
        for i, (traj, traj_deriv, time, f, g) in enumerate(zip(S["states"], S["derivs"], S["times"], S["f_vals"], S["g_vals"])):
            ...
            if len(goal_inds[0]) == 0 or len(unsafe_inds[0])>0:
                true_violated += 1
            # We should check for value violations, but currently don't
            if any(losses > 0):
                violated += 1
...
class Direct_control(Certificate):
    ...
    def learn(
            ...
            if state_sol:
                ...
                if len(supp_samples) == 0:
                        ...
                        if max_loss <= self.margin:
                            ...
                            if self.config.TRACK_WEIGHT > 0 and any(p.requires_grad for p in learners[1].parameters()):
                                ...
                                for u_t in range(learn_loops - t - 1):
                                    ...
                                    if tl.item() < 1e-4:
                                        break
                            if _certify_frozen and best_loss_v3 > self.margin:
                                u1 = learners[1](samples_with_nexts)
                                for v3_t in range(learn_loops):
                                    for opt in optimizer:
                                        opt.zero_grad()
                                    v3_nexts = (f_samples.mT + torch.bmm(g_samples.mT, u1.detach().mT)).mT
                                    V3_next = learners[2](v3_nexts.squeeze(2)).unsqueeze(1).unsqueeze(1)
                                    ...
                                    if len(supp_samples_v3) == 0:
                                        ...
                                        if v3_supp_loss < best_loss_v3:
                                            best_loss_v3 = v3_supp_loss.item()
                                            best_nets_v3 = copy.deepcopy(learners[2])
                                        if v3_supp_loss <= self.margin:
                                            v3_true_max = torch.max(v3_losses, 0)
                                            v3_true_max = v3_true_max[0]
                                            if v3_true_max <= self.margin:
                                                best_loss_v3 = v3_true_max.item()
                                                best_nets_v3 = copy.deepcopy(learners[2])
                                                cert_log.info("V3 converged at step {}".format(v3_t))
                                                break
                                            else:
                                                v3_true_ind = torch.max(v3_losses, 0)[1].item()
                                                supp_samples_v3 = supp_samples_v3.union(set([v3_true_ind]))
                                                v3_active = v3_true_max
                                        else:
                                            v3_active = v3_supp_loss
                                    if v3_active.item() < best_loss_v3:
                                        best_loss_v3 = v3_active.item()
                                        best_nets_v3 = copy.deepcopy(learners[2])
                                    v3_active.backward()
                                    optimizer[2].step()
                                    if v3_t % 500 == 0:
                                        cert_log.debug("v3 train: {:.6f} at step {} (best={:.6f})".format(v3_active.item(), v3_t, best_loss_v3))
                            best_nets = copy.deepcopy(learners)
                            break
                    ...
                    if _certify_frozen:
                        ...
                        v3_max_loss = v3_max_loss[0]
                        supp_samples_v3.add(v3_ind_max)
                        if v3_max_loss.item() < best_loss_v3:
                            best_loss_v3 = v3_max_loss.item()
                            best_nets_v3 = copy.deepcopy(learners[2])
        ...
        if _certify_frozen:
            ...
            all_supps.discard(-1)
            return {ScenAppStateKeys.loss: max_loss.detach(), "best_loss":best_loss_v3, ScenAppStateKeys.v1_best_loss: best_loss, "best_net":(best_nets[0], best_nets[1], best_nets_v3), "new_supps": all_supps}
        return {ScenAppStateKeys.loss: max_loss.detach(), "best_loss":best_loss, "best_net":best_nets, "new_supps": supp_samples}
    def get_violations(self, nets, S, state_data):
        violated = 0
        true_violated = 0
...
class Dissipativity(Certificate):
    ...
    def compute_state_loss(self, Q, S, R, V_D, V_G, V_I, beta):
        ...
        if loss == 0:
            ...
            acc = sum(relud_losses <= 0)/(N_data)
            #acc = (acc*N_data+ sum(relud_losses <= 0))/(2*N_data)
            loss = loss+relud_losses.mean()
            u_max=1 # this shouldn't be hardcoced in future
            control = relu(torch.abs(torch.bmm(S,torch.inverse(R)).max(axis=1)[0])-u_max)
            loss = loss+control.mean()
            acc = (acc*N_data+ sum(control<=0))/(2*N_data)
        else:
            loss = loss + 1
        return loss, {"state_acc":acc.item()*100}
    ...
    def compute_loss(
        ...
        big_nexts = torch.vstack([torch.vstack((n,t_next)) for (n, t_next) in zip(nexts, traj_nexts)])
        L = torch.vstack([torch.vstack((L_n,L_i)) for L_n, L_i in zip(L_next, L)])
        Vdot = torch.vstack([torch.vstack((V_n-V_elem,Vdot_elem)) for V_n, V_elem, Vdot_elem in zip(V_next, V,Vdot)])

        num_repeats = nexts.shape[1]+1
        ...
        L = L.unsqueeze(2).mT
        big_nexts = big_nexts.unsqueeze(2).mT
        Vdot = Vdot.unsqueeze(2)
        ...
        #
        #loss = V_next-V+torch.bmm(torch.bmm(S, torch.inverse(R)), S.mT)-Q+0*relu(L_next[0,0,0])
        #loss = V_next-V+0*relu(L_next[0,0,0])
    ...
    def learn(
        ...
        for t in range(learn_loops):
            if state_sol:
                ...
                V2 = learners[0](states_only)
                Q2 = learners[1](states_only)[:i1-idot1]
                S2 = learners[2](states_only)[:i1-idot1]
                R2 = learners[3](states_only)[:i1-idot1]
                ...
                #V_next = torch.learners[0](nexts)
                V_next = torch.unsqueeze(V_next, 2)
                ...
                loss, state_acc = self.compute_state_loss(Q2, S2, R2, V_D, V_G, V_I, beta)
                acc = learn_accuracy|state_acc
                losses = relu(losses) + loss
                #if loss > 0:
                #    losses = relu(losses) + loss
                ...
                while True:
                    ...
                    V_G = V2[i1+i2+i3-idot1-idot2-idot3:i1+i2+i3+i4-idot1-idot2-idot3-idot4]
                    beta = V_SG.min()
                    loss,_ = self.compute_state_loss(Q, Smat, R, V_D, V_G, V_I, beta)
                    #if state_itt % 100 == 0:
                    #    import pdb; pdb.set_trace()
                    if loss == 0:
                        state_sol=True
        ...
        V2 = best_nets[0](states_only)
        Q2 = best_nets[1](states_only)
        S2 = best_nets[2](states_only)
        R2 = best_nets[3](states_only)[:i1-idot1]
        ...
        loss,_ = self.compute_state_loss(Q2, S2, R2, V_D, V_G, V_I, beta)
    ...
    def learn(
        ...
        for t in range(learn_loops):
            ...
            if state_sol:
                ...
                if (max_loss-best_loss) >= tol: #converged, only occurs for DT if we have ReLU on lie
                    if sorted_keys[0] in supp_samples:
                        break
                    else:
                        supp_samples = supp_samples.union(set([sorted_keys[0]]))
...
class RWS(Certificate):
    ...
    def compute_loss(self, V_i, V_u, V_d, V_d_states, V_g, Vdot_d, beta, indices, supp_samples):
    ...
    def learn(
        ...
        for t in range(learn_loops):
            ...
            if state_sol:
                ...
                if (max_loss-best_loss) >= 1e-1: #converged, only occurs for DT if we have ReLU on lie
                    if sorted_keys[0] in supp_samples:
                        break
                    else:
                        supp_samples = supp_samples.union(set([sorted_keys[0]]))
...
class RSWS(RWS):
    ...
    def compute_beta_loss(self, beta, V_g_border_min, V_g, Vdot_g, V_d, indices, supp_samples):
...
class AutoSets:
    ...
    def auto(self) -> (dict, dict):
        ...
        if self.certificate == CertificateType.DIRECTCONTROL:
            return self.auto_direct_control()
        elif self.certificate == CertificateType.DIRECTCONTROLBARR:
            return self.auto_direct_control_barr()
        elif self.certificate == CertificateType.DIRECTCONTROLRWA:
            return self.auto_direct_control_rwa()
...
def get_certificate(
    ...
    if certificate == CertificateType.DIRECTCONTROL:
        return Direct_control
    elif certificate == CertificateType.DIRECTCONTROLBARR:
        return Direct_control_barr
    elif certificate == CertificateType.DIRECTCONTROLRWA:
        return Direct_control_RWA
