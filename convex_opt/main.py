...
def Jet_engine(N):
    ...
    constraints = []
    for state, next_s, time in zip(all_data[1],all_data[2], all_data[0]):
        ...
        if len(initial_inds[0])>1:
            constraints.append(W_2@cp.pos(W_1@state[:, initial_inds[0]]+b_1)+b_2<= eta)
        elif len(initial_inds[0]) == 1:
            constraints.append(W_2@cp.pos(W_1@state[:, initial_inds[0], np.newaxis]+b_1)+b_2<= eta)
        time = np.expand_dims(time,0)
        if len(unsafe_inds[0]) > 1:
            constraints.append(-(W_2@cp.pos(W_1@state[:, unsafe_inds[0]]+b_1)+b_2)<= eta)
        elif len(unsafe_inds[0]) == 1:
            constraints.append(-(W_2@cp.pos(W_1@state[:, unsafe_inds[0], np.newaxis]+b_1)+b_2)<= eta)
        constraints.append((W_2@cp.pos(W_1@next_s+b_1)+b_2-(W_2@cp.pos(W_1@state+b_1)+b_2))/time<= eta)
    import pdb; pdb.set_trace()
